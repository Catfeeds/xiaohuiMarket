            ...
            <div class="card-box">
                <form class="form-inline" role="form" action="${base}/purchase/supplierPage.do">
                    <div class="form-group m-l-15">
                        <label>供应商名称：</label>
                        <input type="text" class="form-control" id="supplierName" placeholder="输入供应商名称" name="supplierName" value="${supplierName}">
                    </div>
                    <div class="form-group m-l-15">
                        <label>供应商属性：</label>
                        <select id="supplierSelect" class="form-control" name="supplierType">
                            <option value="-1" <#if supplierType?exists && supplierType == -1>selected</#if>>所有供应商</option>
                            <option value="1" <#if supplierType?exists && supplierType == 1>selected</#if>>一级供应商</option>
                            <option value="2" <#if supplierType?exists && supplierType == 2>selected</#if>>二级供应商</option>
                            <option value="3" <#if supplierType?exists && supplierType == 3>selected</#if>>品牌供应商</option>
                            <option value="4" <#if supplierType?exists && supplierType == 4>selected</#if>>超市供应商</option>
                        </select>
                    </div>
                    <div class="form-group m-l-15">
                        <label>状态：</label>
                        <select id="status" class="form-control" name="status">
                            <option value="-1" <#if status?exists && status == -1>selected</#if>>全部</option>
                            <option value="0" <#if status?exists && status == 0>selected</#if>>停用</option>
                            <option value="1" <#if status?exists && status == 1>selected</#if>>正常</option>
                        </select>
                    </div>
                    <div class="form-group m-l-15">
                ...
                $(this).on('click', function () {
                    $.post("${base}/purchase/updateSupplierStatus.do?id=" +$(that).attr("data_id")+ "&status=1", function(data) {
                        ...
                        if(data.code == "0") {
                            ...
                            setTimeout(function(){
                                location.href = "${base}/purchase/supplierPage.do?pageIndex=${pageIndex}";
                            },1000);
                        } else {
