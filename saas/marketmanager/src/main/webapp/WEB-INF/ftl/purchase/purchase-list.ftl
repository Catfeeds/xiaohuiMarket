                ...
                <form class="form-inline" role="form" action="${base}/purchase/purchasePage.do">
                        ...
                        <select id="houseSelect" class="form-control" name="warehouseCode">
                            ...
                            <#if warehouseItem?exists >
                                <#list warehouseItem as warehouse>
                                    <option value="${warehouse.warehouseCode}" <#if warehouseCode?exists && warehouseCode == warehouse.warehouseCode>selected</#if>>
                    ...
                    <div class="form-group m-l-15">
                        ...
                        <select id="status" class="form-control" name="status">
                            ...
                            <option value="0" <#if status?exists && status == 0>selected</#if>>未提交</option>
                            <option value="1" <#if status?exists && status == 1>selected</#if>>未入库</option>
                            <option value="2" <#if status?exists && status == 2>selected</#if>>入库异常</option>
                            <option value="3" <#if status?exists && status == 3>selected</#if>>完成入库</option>
                            <option value="4" <#if status?exists && status == 4>selected</#if>>强制完成入库</option>
                            <option value="5" <#if status?exists && status == 5>selected</#if>>部分入库</option>
                        </select>
                    </div>
                    <div class="form-group m-l-15">
