    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">商品阶梯价编辑</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-10">
                        <p id="priceItemName"></p>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="number" id="iMin" class="form-control">
                                <span class="input-group-addon">件 至 最大</span>
                                <input type="number" id="iMax" class="form-control">
                                <span class="input-group-addon">件，每件 </span>
                                <input type="text" id="iPrice" class="form-control" onkeyup="clearNoNum(this)">
            ...
            <div class="modal-footer">
                <div class="row">
                    <div class="col-sm-12">
                        <button id="priceOkBtn" type="button" class="btn btn-primary"
                            ...
                            <div class="input-group m-l-15">
                                <div class="input-group-btn">
                                    ...
                                    <ul class="dropdown-menu" id="searchType">
                                        <li><a href="javascript:void(0)" search_name="商品模板名称"
                                               search_type="itemTemplateName">按商品模板名称搜索</a>
                                        </li>
                                        <li><a href="javascript:void(0)" search_name="商品模板ID"
                                               search_type="itemTemplateId">按商品模板ID搜索</a>
                                        </li>
                                    </ul>
                                </div>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <#if items?exists && (items?size > 0)>
                            ...
                            <tr>
                                ...
                                <#if marketId == 0>
                                    <td><a href="${base}/market/marketItems.do?marketId=${item.ownerId?c}"
                                           alt="点击查看该店铺的商品列表">${item.ownerId?c}</a></td>
                                </#if>
                                ...
                                <td>
                                    ...
                                    <button id="editPriceBtn" type="button" item_id="${item.id?c}" data_id="0"
                                            class="btn waves-effect waves-light btn-info btn-sm">编辑满减
                                    </button>
                                </td>
                            </tr>
                            </#list>
            ...
            <#if items?exists && (items?size > 0) >
                ...
                $("#itemsTable").find('button[id=editBtn]').each(function () {
                    $(this).on('click', function () {
                        var s_searchKey = _searchKeyTxt.val();
                        open({
                            url: "${base}/market/marketItemEdit.do?id=" + $(this).attr("data_id")
                ...
                //单项满减功能
                $("#itemsTable").find('button[id=editPriceBtn]').each(function () {
                    $(this).on('click', function () {

                        var btn = $(this);
                        editItemBtn = btn;//记录

                        var data_id = btn.attr("data_id");
                        var itemid = btn.attr("item_id");

                        $("#priceOkBtn").attr("item_id", itemid);
                        $("#priceOkBtn").attr("data_id", data_id);

                        if(data_id != 0) {
                            //已存在满减，则直接拿参数
                            var iMin = btn.attr("iMin");
                            var iMax = btn.attr("iMax");
                            var iPrice = btn.attr("iPrice");
                            ...
                            //请求服务器，获取原满减情况
                            tokenPost("${base}/market/getItemLadderPrices.do?itemId=" + itemid, function (json) {
                                //没有该满减，则直接打开
                                if (json.code != 0) {
                                    $("#iMin").val("");
                                    $("#iMax").val("");
                                    $("#iPrice").val("");
                                    $("#priceOkBtn").attr("data_id", 0);
                                } else {
                                    var item = json.response.datas;
                                    $("#iMin").val(item.minQuantity);
                                    $("#iMax").val(item.maxQuantity);
                                    $("#iPrice").val(item.expectPrice);
                                    $("#priceOkBtn").attr("data_id", item.id);
                                }
                                $("#priceDialog").modal('show');
                            }, "json");
                        }
                ...
                $("#priceOkBtn").on('click', function () {
                    ...
                    tokenPresPost("${base}/market/marketItemUpdateActivity.do", post_data, function (json) {
                        ...
                        if (json.code != 0) {
                            ...
                            var item = json.response.datas;
                            //转入按钮
                            editItemBtn.attr("iMin", item.minQuantity);
                            editItemBtn.attr("iMax", item.maxQuantity);
                            editItemBtn.attr("iPrice", item.expectPrice);
                            editItemBtn.attr("data_id", item.id);
                            editItemBtn.attr("item_id", item.itemId);
                            swal("操作成功");
                        }
                    }, "json");
                });
