...
public class BasicRemoteService {
    ...
    protected static JSONObject postAdvertiseMsg(String url, Map<String, String> parameters) {
        try {
            // 填充签名参数 参数key为：sign 参与的密钥字段名：key
            CommonUtils.fillSignature(parameters, ConfigFactory.getXMarketConfig().getOrderAppkey());
            String data = HttpRequest.post(ConfigFactory.getDomainNameConfig().advertiseRemoteURL + url, parameters);
